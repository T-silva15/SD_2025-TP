    ...
    static void GuardarNoBD(Dictionary<string, object> dados)
    {
        lock (dbLock)
        {
            ...
            using (var con = new SqlConnection(connectionString))
            {
                ...
                var jsonData = dados["Data"].ToString();
                var dataList = JsonSerializer.Deserialize<List<Dictionary<string, object>>>(jsonData);

                foreach (var item in dataList)
                {
                    string tipo = item["DataType"].ToString();
                    long totalValue = Convert.ToInt64(item["TotalValue"]);
